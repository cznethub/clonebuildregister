...
class TestExceptions(unittest.TestCase):
    ...
    def test_clone_repo_fail_with_clonename(self):
        ...
        """
        self.assertRaises(CloneRepositoryException, clone_repo, "meow", \
                          "this_repo_don'texists", "1232132tttt", "oopsies") 
    ...
    def test_push_image_to_gcloud_fail(self):
        ...
        self.assertRaises(GCloudRegisterImageException, register_image, "testimage", "v1", "testimage", "v1", "notaregion", \
                          "sillyprojectid", "notarepository")
        
        client.images.remove("testimage:v1", force=True)
        client.images.remove("continuumio/miniconda3:latest", force=True)
        client.images.remove("notaregion-docker.pkg.dev/sillyprojectid/notarepository/testimage:v1", force=True)
        
    def test_bad_copy_env(self):
        """ Test tries to copy env to remote environment, but fails to do so due to faulty path"""
        self.assertRaises(BadCopyEnvException, build_image, "meow:v1", "clonebuildregister/test", \
                          "clonebuildregister/test/buildImageTest.env", "sillygoofy/!~")
    
    def test_CBRException_del_repository_clone_name(self):
        """ Test shows CBR exception, """
        
        self.assertRaises(CBRException, clonebuildregister, GITHUB_REPO_ORG, GITHUB_REPO_NAME, GITHUB_REPO_BRANCH, "testimage", "v1", "funny",
                          "testimage", "v1", "sillylocation", "notgood", f"{GCLOUD_REPOSITORY}", delete_repository=True, clone_name="funny")
        client = docker.from_env()
        self.assertTrue(len(client.images.list(name="testimage")) > 0)
    ...
    def test_CBRException_del_repository(self):
        """ Test shows CBR exception, """
                   
        self.assertRaises(CBRException, clonebuildregister, GITHUB_REPO_ORG, GITHUB_REPO_NAME, GITHUB_REPO_BRANCH, "testimage", "v1", GITHUB_REPO_NAME,
                          "testimage", "v1", "sillylocation", "notgood", f"{GCLOUD_REPOSITORY}", delete_repository=True)
        client = docker.from_env()
        self.assertTrue(len(client.images.list(name="testimage")) > 0)
