...
class TestBuildImage(unittest.TestCase):
    ...
    def test_clonebuildregister_delete_repository(self):
        ...
        response = clonebuildregister.clonebuildregister(GITHUB_REPO_ORG, GITHUB_REPO_NAME, GITHUB_REPO_BRANCH, "testimage", "v1", GITHUB_REPO_NAME,
                          "testimage", "v1", f"{GCLOUD_LOCATION}", f"{GCLOUD_PROJECT}", f"{GCLOUD_REPOSITORY}", delete_repository=True
                          )
        client = docker.from_env()
        self.assertTrue(len(client.images.list(name="testimage")) > 0)

        self.assertFalse(os.path.exists(GITHUB_REPO_NAME)) # we need to make sure that repository is deleted after program finishes
    ...
    def test_clonebuildregister_delete_repository_with_clone_name(self):
        ...
        response = clonebuildregister.clonebuildregister(GITHUB_REPO_ORG, GITHUB_REPO_NAME, GITHUB_REPO_BRANCH, "testimage", "v1", test_repo_name,
                          "testimage", "v1", f"{GCLOUD_LOCATION}", f"{GCLOUD_PROJECT}", f"{GCLOUD_REPOSITORY}", delete_repository=True, \
        ...
        self.assertFalse(os.path.exists(test_repo_name)) # we need to make sure that repository is deleted after program finishes
